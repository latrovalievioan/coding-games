...
const calcDir = (humansWithNearestZombies: HumanWithNearestZombie[]) => {
  const leftHumans = humansWithNearestZombies.filter(h => h.human.y < MID_Y)
  const rightHumans = humansWithNearestZombies.filter(h => h.human.y >= MID_Y)
...
while (true) {
  ...
  console.log(`${calcDir(filterUnsavableHumans(humansWithNearestZombies))} go6oooo`);
}
