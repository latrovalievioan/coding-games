...
const killZombies = (humans: Entity[], zombies: Entity[], ash: Entity) => {
  ...
  for (let i = 0; i < zombies.length; i++) {
    ...
    console.error({ash, zombie, dist: calcVecDist(zombie,ash)})
  ...
  console.error({killedZombies,aliveZombies})
...
const runSimulation = (
  ...
  if ((bestScoreWithSteps.score <= score)) {
  ...
  const indexOfTargetZombie = aliveZombies.findIndex(x => x.id === targetZombie.id)

  const scoreAfterRound = aliveHumans.length ? scoreFromKills + score : 0

  if(indexOfTargetZombie > -1) {
      runSimulation(aliveHumans, aliveZombies, ash, scoreAfterRound, aliveZombies[indexOfTargetZombie], [...steps, aliveZombies[indexOfTargetZombie]])
  }else {
      const zombie = aliveZombies[0];
      runSimulation(
        aliveHumans,
        aliveZombies,
        ashAfterMove,
