...
const moveAsh = (ash: Entity, targetHuman: Entity) =>
  calcFrameMove(ASH_SPEED, ash, targetHuman) as Entity;
...
const runSimulation = (
  ...
  const ashAfterMove = moveAsh(ash, targetHuman);
...
while (true) {
  ...
  if (i === 0) {
    for (let i = 0; i < humans.length; i++) {
      ...
      runSimulation(humans, zombies, ash, 0, human, [human]);
    }
  }
